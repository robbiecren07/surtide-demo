...
export async function getFeaturedSingle() {
  const query = `
  {
    collection(handle: "featured-single") {
      title
      products(first: 1) {
        edges {
          node {
  ...
  const featuredProduct = response.data.collection.products.edges ? response.data.collection.products.edges : []
...
export async function getAllCollections() {
  ...
  const collections = response.data.collections.edges ? response.data.collections.edges : []
